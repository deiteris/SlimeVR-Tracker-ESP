...
void VQF::updateGyr(const float gyr[3])
{
    ...
    if (params.restBiasEstEnabled || params.magDistRejectionEnabled) {
        ...
        if (state.restLastSquaredDeviations[0] >= square(params.restThGyr*float(M_PI/180.0))
...
void VQF::updateMag(const float mag[3])
{
    ...
    if (params.restBiasEstEnabled) {
        ...
        float magNormSquared = square(state.restLastMagLp[0]) + square(state.restLastMagLp[1]) + square(state.restLastMagLp[2]);
    ...
    if (params.magDistRejectionEnabled) {
        ...
        if (fabs(state.magNormDip[0] - state.magRefNorm) < params.magNormTh*state.magRefNorm
                && fabs(state.magNormDip[1] - state.magRefDip) < params.magDipTh*float(M_PI/180.0)) {
        ...
        if (fabs(state.magNormDip[0] - state.magCandidateNorm) < params.magNormTh*state.magCandidateNorm
                && fabs(state.magNormDip[1] - state.magCandidateDip) < params.magDipTh*float(M_PI/180.0)) {
    ...
    if (state.lastMagDisAngle > float(M_PI)) {
        state.lastMagDisAngle -= float(2*M_PI);
    } else if (state.lastMagDisAngle < float(-M_PI)) {
        state.lastMagDisAngle += float(2*M_PI);
    }
    ...
    if (state.delta > float(M_PI)) {
        state.delta -= float(2*M_PI);
    } else if (state.delta < float(-M_PI)) {
        state.delta += float(2*M_PI);
    }
}
...
float VQF::getBiasEstimate(float out[3]) const
{
    ...
    float sum1 = fabs(state.biasP[0]) + fabs(state.biasP[1]) + fabs(state.biasP[2]);
    float sum2 = fabs(state.biasP[3]) + fabs(state.biasP[4]) + fabs(state.biasP[5]);
    float sum3 = fabs(state.biasP[6]) + fabs(state.biasP[7]) + fabs(state.biasP[8]);
    float P = std::min(std::max(std::max(sum1, sum2), sum3), coeffs.biasP0);
#else
    float P = state.biasP;
#endif
    // convert standard deviation from 0.01deg to rad
    return sqrt(P)*float(M_PI/100.0/180.0);
}
...
void VQF::setBiasEstimate(float bias[3], float sigma)
{
    ...
    if (sigma > 0) {
        float P = square(sigma*float(180.0*100.0/M_PI));
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
...
void VQF::getRelativeRestDeviations(float out[3]) const
{
    out[0] = sqrt(state.restLastSquaredDeviations[0]) / (params.restThGyr*float(M_PI/180.0));
...
void VQF::setRestDetectionThresholds(float thGyr, float thAcc, float thMag)
{
...
void VQF::quatApplyDelta(float q[], float delta, float out[])
{
...
void VQF::filterCoeffs(float tau, float Ts, double outB[], double outA[])
{
    assert(tau > 0);
    assert(Ts > 0);
...
void VQF::filterInitialState(float x0, const double b[3], const double a[2], double out[])
{
...
void VQF::filterAdaptStateForCoeffChange(float last_y[], size_t N, const double b_old[],
...
void VQF::setup()
{
...
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
    float pMotion = square(params.biasSigmaMotion*100.0);
    ...
    float pRest = square(params.biasSigmaRest*100.0);
